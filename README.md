...
Папка `3-stage/` содержит Jupyter notebook для локального просмотра слайд-шоу предсказаний. Положите модель в `3-stage/best_model.pth`. КТ-срезы, маски и `manifest.csv` остаются в корневой папке `preprocessed_npy/` или `prepared_npy/`. Notebook случайно выбирает из `manifest.csv` пациента с опухолевым срезом, загружает его срезы и формирует встроенное HTML5-видео. Каждый кадр состоит из трёх слоёв: CT-срез, синяя истинная маска и красная предсказанная маска. Для текущего кадра выводятся `Dice`, `Precision` и `Recall`.
